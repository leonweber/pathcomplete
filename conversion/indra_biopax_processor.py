...
from pybiopax import model_to_owl_file, model_from_owl_file
...
class BiopaxProcessor(object):
    ...
    def _control_conversion_iter(self, conversion_type, controller_logic):
        ...
        assert controller_logic in {'complex', 'primary', 'all'}
        ...
        for control in self.model.get_objects_by_type(bp.Control):
            ...
            ev = self._get_text_evidence(control.controlled)
            for controller_pe in control.controller:
                ...
                elif controller_logic == 'complex':
                    primary_controller = self._get_complex_controller(controller_pe, control)
                else:
                    primary_controller = \
                ...
                for pc in _listify(primary_controller):
                    if isinstance(pc, Agent):
                        primary_controller_agents.append(pc)
                    else:
                        primary_controller_agents += \
    ...
    def _conversion_no_control_iter(self):
        ...
        for conversion in self.model.get_objects_by_type(bp.Conversion):
            ...
            # if i > 1000:
            #     break
            ev = self._get_text_evidence(conversion)
    ...
    def _conversion_state_iter(self):
        ...
        for primary_controller_agent, ev, control, conversion in \
                self._control_conversion_iter(bp.Conversion, 'complex'):
            for inp, outp in self.find_matching_left_right(conversion):
                ...
                for inp_simple, outp_simple in \
                    ...
                    inp_agents = \
                        self._get_agents_from_singular_entity(inp_simple)
                    for inp_agent in inp_agents:
                        self._add_bound_conditions(agent=inp_agent,
                                                   pe_simple=inp,
                                                   pes_complex=conversion.left)
    ...
    def get_regulate_amounts(self):
        ...
        for subj, ev, control, conversion in \
                self._control_conversion_iter(bp.TemplateReaction, 'complex'):
    ...
    def _get_agents_from_singular_entity(self, bpe: bp.PhysicalEntity):
        ...
        # loc = BiopaxProcessor._get_entity_loc(bpe)
        loc = None
        ...
        if nhgnc_ids > 1 and nup_ids == 1:
            for hgnc_id in xrefs['HGNC']:
                ...
                if standard_name:
                    name = standard_name
                agents.append(Agent(name, db_refs=db_refs, mods=mcs, location=loc))
    ...
    def _get_agents_from_complex(self, bpe: bp.PhysicalEntity, primary=None):
        agents = []

        pe_type = infer_pe_type(bpe)
        if pe_type == 'complex':
            complexes = [bpe]
        elif pe_type == "complex_family":
            complexes = bpe.member_physical_entity
        else:
            return bpe

        for complex in complexes:
            members = expand_complex(complex)
            agent_members = []
            for m in members:
                agent_members += self._get_agents_from_singular_entity(m)[:1] # take the first protein as representative for families
            c = agent_members[0]
            for m in agent_members[1:]:
                c.bound_conditions.append(BoundCondition(m))
            agents.append(c)
    ...
    @classmethod
    def _get_entity_loc(cls, bpe):
        return bpe.cellular_location.term[0] if bpe.cellular_location else None

    def _get_complex_controller(self, controller_pe, control):
        return self._get_agents_from_complex(controller_pe)

    def _get_text_evidence(self, bpe):
        if not bpe.comment:
            return None
        max_comment_len = max(len(c) for c in bpe.comment)
        longest_comment = [c for c in bpe.comment if len(c) == max_comment_len][0]
        return Evidence(text=longest_comment)

    def _find_containing_complex(self, pe_simple, pe_complex):
        if pe_simple in pe_complex.component:
            return pe_complex
        else:
            for member in pe_complex.component:
                if _is_complex(member):
                    return self._find_containing_complex(pe_simple, member)
            return None

    def _add_bound_conditions(self, agent, pe_simple, pes_complex):
        cmplx = None
        for pe in pes_complex:
            if _is_complex(pe):
                cmplx = self._find_containing_complex(pe_simple, pe)
                if cmplx:
                    break

        if cmplx:
            for member in expand_complex(cmplx):
                if member != pe_simple:
                    member = self._get_agents_from_singular_entity(member)[0] # take the first protein as representative for families
                    agent.bound_conditions.append(BoundCondition(member))
