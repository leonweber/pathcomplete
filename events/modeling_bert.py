...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.node_type_embeddings = nn.Embedding(config.node_type_vocab_size, config.hidden_size)
        #self.edge_type_embeddings = nn.Embedding(config.edge_type_vocab_size, config.hidden_size)

        # self.LayerNorm is not snake-cased to stick with TensorFlow model variable name and be able to load
        # any TensorFlow checkpoint file
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.edge_layer_norm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None,
                node_type_ids=None, edge_type_ids=None):
        ...
        if node_type_ids is None: # default to zero to retain default BERT without node_type_ids
            node_type_embeddings = torch.zeros(input_shape + (self.node_type_embeddings.embedding_dim,),
                                               dtype=torch.long, device=device)
        else:
            node_type_embeddings = self.node_type_embeddings(node_type_ids)

        # if edge_type_ids is None: # default to zero to retain default BERT without edge_type_ids
        #     batch_size, length = input_shape
        #     edge_type_embeddings = torch.zeros(
        #         (batch_size, length, length, self.edge_type_embeddings.embedding_dim),
        #         dtype=torch.long, device=device).to(input_ids.device).float()
        # else:
        #     edge_type_embeddings = self.edge_type_embeddings(edge_type_ids)
        ...
        embeddings = inputs_embeds + position_embeddings + token_type_embeddings + node_type_embeddings
        embeddings = self.LayerNorm(embeddings)
        embeddings = self.dropout(embeddings)

        # edge_type_embeddings = self.edge_layer_norm(edge_type_embeddings)
        # edge_type_embeddings = self.dropout(edge_type_embeddings)
...
class BertSelfAttention(nn.Module):
    ...
    def transpose_edge_embeddings_for_scores(self, x):
        new_x_shape = x.size()[:-1] + (self.num_attention_heads, self.attention_head_size)
        x = x.view(*new_x_shape)
    ...
    def forward(
        ...
        # key_edge_embeddings = self.transpose_edge_embeddings_for_scores(self.key(edge_type_embeddings))
        # value_edge_embeddings = self.transpose_edge_embeddings_for_scores(self.value(edge_type_embeddings))

        # Following Shaw, Uszkoreit & Vaswani 2018
        # FIXME Above paper has one edge embedding for query and another one for value
        # We emulate this by using the key and value transformations
        # edge_type_scores = key_edge_embeddings * query_layer.unsqueeze(3)
        # edge_type_scores = edge_type_scores.sum(dim=-1)


        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))  #+ edge_type_scores
        ...
        # edge_type_values = attention_probs.unsqueeze(-1) * value_edge_embeddings
        # edge_type_values = edge_type_values.sum(dim=2)

        context_layer = torch.matmul(attention_probs, value_layer) #+ edge_type_values
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        self_attention_outputs = self.attention(hidden_states, attention_mask, head_mask, edge_type_embeddings=edge_type_embeddings)
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_outputs = layer_module(
                hidden_states, attention_mask, head_mask[i], encoder_hidden_states, encoder_attention_mask, #edge_type_embeddings=edge_type_embeddings
            )
...
class BertGNNModel(BertPreTrainedModel):
    def __init__(self, config):
    ...
    Examples::
        ...
        embedding_output  = self.embeddings(
            input_ids=input_ids, position_ids=position_ids, token_type_ids=token_type_ids, inputs_embeds=inputs_embeds, node_type_ids=node_type_ids, #edge_type_ids=edge_type_ids
        )
