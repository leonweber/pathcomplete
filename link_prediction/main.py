...
class Experiment:
    ...
    def predict_all(self, model):
        ...
        for relation in self.relations:
            ...
            for i in range(0, len(self.entities), self.batch_size):
                e1_idx = torch.arange(i, min(i+self.batch_size, len(self.entities))).long()
                r_idx = (torch.ones_like(e1_idx) * self.relation_idxs[relation]).long()
        ...
        with open('predictions.json', 'w') as f:
            json.dump(all_predictions, f, indent=1)
    ...
    def evaluate(self, model, data):
        ...
        for i in range(0, len(test_data_idxs), self.batch_size):
            ...
            for j in range(data_batch.shape[0]):
                rank = np.where(sort_idxs[j]==e2_idx[j].cpu().numpy())[0][0]
                ranks.append(rank+1)
...
if __name__ == '__main__':
    ...
    experiment = Experiment(num_iterations=args.num_iterations, batch_size=args.batch_size, learning_rate=args.lr, 
                            decay_rate=args.dr, ent_vec_dim=args.edim, rel_vec_dim=args.rdim, cuda=True,
