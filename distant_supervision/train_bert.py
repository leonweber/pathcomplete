...
def train(args, train_dataset, model):
    ...
    for _ in train_iterator:
        ...
        for step, batch in epoch_iterator:
            ...
            direct_labels = torch.cat([batch['is_direct'], torch.tensor([0]).to(logits.device)]).long()
            ...
            if is_direct.sum() > 0:
                direct_ap = average_precision_score(direct_labels.cpu().numpy(), meta['alphas'].cpu().detach().numpy().ravel(),
                                                    average='micro')
